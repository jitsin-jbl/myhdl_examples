...
class SIIR():
    ...
    def __init__(self, 
        ...
        if b is not None and a is not None:
            ...
            Wn = float(Fc)/(Fs/2.0) 
            self.b, self.a  = iirfilter(N, Wn, btype='lowpass')
            self.nSection = 1
    ...
    def PlotResponse(self, fn="siir"):
        ...
        pylab.legend(('IIR Design', 'IIR Desing Quantized Coefficients',
                      'Fixed-Point Sim', 'Floating-Point Sim'))
...
if __name__ == '__main__':
    ...
    print "Simulation"
    tbtrace = traceSignals (flt.TestFreqResponse,Nloops=3, Nfft=1024)
    tb = flt.TestFreqResponse(Nloops=3, Nfft=1024)
    #sim = Simulation(tb)
    sim = Simulation(tbtrace)
