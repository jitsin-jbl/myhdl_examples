...
wire gfifo_fbus_clear;

reg [35:0] gfifo_mem [0:16-1];

assign gfifo_fbus_clear = 0;
...
always @(posedge clock) begin: M_FIFO_SHORT_GFIFO_RTL_SRL_IN
    integer ii;
    if (wr) begin
        gfifo_mem[0] <= datain;
...
always @(posedge clock, posedge reset) begin: M_FIFO_SHORT_GFIFO_RTL_FIFO
    if (reset == 1) begin
        fbus_empty <= 1;
        fbus_full <= 0;
        gfifo_addr <= 0;
...
always @(posedge clock, posedge reset) begin: M_FIFO_SHORT_GFIFO_DBG_OCCUPANCY
    if (reset == 1) begin
        gfifo_nvacant <= 16;
        gfifo_ntenant <= 0;
    end
